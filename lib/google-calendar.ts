...
function parseIcsEvents(text: string, calendarId: string): CalendarEvent[] {
  ...
  for (const line of lines) {
    ...
    if (line === "END:VEVENT") {
      if (current) {
        events.push({
          ...
          start_at: current.DTSTART ? parseIcsDate(current.DTSTART) : null,
          end_at: current.DTEND ? parseIcsDate(current.DTEND) : null,
    ...
    if (key) current[key] = decodeIcsText(getIcsValue(line))
  }
...
export async function fetchGoogleCalendarEvents() {
  ...
  return apiKey
    ? fetchEventsFromApi(calendarId, apiKey)
    : fetchEventsFromIcs(calendarId)
}
