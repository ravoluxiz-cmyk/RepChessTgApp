...
const BUCKET = process.env.SUPABASE_CLUB_CONTENT_BUCKET || "club-content"
...
async function ensureClubContentBucket() {
  ...
  if (error) {
    console.error("Failed to create club content bucket:", error)
    return false
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!(file instanceof File)) {
      return NextResponse.json({ error: "Файл изображения обязателен" }, { status: 400 })
    }
    ...
    const bucketReady = await ensureClubContentBucket()
    if (!bucketReady) {
      return NextResponse.json({ error: "Не удалось подготовить хранилище изображений" }, { status: 500 })
    }

    const extension = getExtension(file)
    const path = `club/${Date.now()}-${crypto.randomUUID()}.${extension}`
    ...
    if (error || !data) {
      console.error("Failed to upload club content image:", error)
    ...
    console.error("Club content upload failed:", error)
