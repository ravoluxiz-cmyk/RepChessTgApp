...
export function PosterUploadField({
  ...
  async function uploadFile(file: File) {
    ...
    try {
      ...
      const response = await fetch("/api/tournaments/poster", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(data.error || "Не удалось загрузить афишу")
      }

      onChange(data.url)
    } catch (err) {
      setError(err instanceof Error ? err.message : "Не удалось загрузить афишу")
    } finally {
      setUploading(false)
  ...
  return (
    <div className="space-y-3">
      <label className={labelClassName}>Афиша</label>

      {value && (
        <div className="overflow-hidden rounded-lg border border-white/10 bg-black/20">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={value} alt="Афиша турнира" className="aspect-[16/9] w-full object-cover" />
        </div>
      )}
      ...
      <div className="flex items-start gap-2 text-xs text-white/50">
        ...
        <span>JPG, PNG, WebP или GIF до 8 МБ. Можно также вставить прямую ссылку ниже.</span>
      </div>
